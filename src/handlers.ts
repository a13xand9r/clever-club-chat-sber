...
export const questionHandler: ScenarioHandler = async ({ req, res, session }, dispatch) => {
    ...
    res.setPronounceText(`${session.isFirstQuestion ? 'Первый вопрос: ' : ''}${session.currentQuestion?.question}`)
    res.appendBubble(`${session.isFirstQuestion ? 'Первый вопрос: ' : ''}${changeBrackets(session.currentQuestion?.question as string)}`)
